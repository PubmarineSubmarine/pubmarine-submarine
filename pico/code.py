...
MOTOR_MIN_START = 0.4
MOTOR_MAX_START = 0.4
MOTOR_MAX_CHANGE_TICK = 0.01
#MOTOR_MAX_CHANGE_TICK = 0.1
SV1_ADJUST = 5
SV2_ADJUST = 0
SV3_ADJUST = 0
SV4_ADJUST = 0
...
def soft_motor_control(motor, requested):
    if motor == controls.motor_x:
        requested *= MOTOR_X_SCALE
    elif motor == controls.motor_z:
        requested *= MOTOR_Z_SCALE
...
def cmd_test(params):
    INTERVAL = 1.0
    cmd_stop("")
    #controls.motor_x.throttle = 1
    controls.sv1.angle = 45
    time.sleep(INTERVAL)
    #controls.motor_x.throttle = 0
    controls.sv1.angle = 90
    #controls.motor_z.throttle = 1
